<?php
	...
	$query .= ' GROUP BY `day`, `month`, `year` order by date asc';
	$qt = rquery($query);
...
/// NEW
	...
	$iPoints = mysqli_num_rows($qt);
	$dx = (GRAPH_WIDTH - 40) / ($iPoints - 1);
	while ($row = mysqli_fetch_row($qt)) {
		$y = $row[0];
		$aCoords[$x] = $y;
		$x+= $dx;
	}
...
*/
	...
	imagettftext($vImage, 10, 0, 30, $iPanelY + 20, $vTextColor, 'ttfonts/FreeSerif.ttf', 'Cubic splines in PHP for graphs:         ' . $sTime . ' sec');
