<?php
	...
	if(check_page_rights(get_page_id_by_filename('raumplanung.php'))) { // Wichtig, damit Niemand ohne Anmeldung etwas ändern kann
		if(get_get('semester')) {
			...
			$objPHPExcel = raumplanung_crazy(get_get('institut'), get_get('semester'), 0);
			if($objPHPExcel) {
				header('Content-type: application/vnd.ms-excel');
				header('Content-Disposition: attachment; filename="crazy-Raumplanung-'.$institut_name.'-'.$semester_name[2].'_'.$semester_name[1].'.xlsx"');
