...
?>
		<div id="accounts">
			<form method="post" enctype="multipart/form-data" action="admin?page=<?php print $GLOBALS['this_page_number']; ?>">
				<input type="text" placeholder="Von..." class="ua_input" name="from" class="datepicker" value="<?php print htmlentities(get_post('from') ?? ""); ?>" />
				<input type="text" placeholder="... bis" class="ua_input" name="to" class="datepicker" value="<?php print htmlentities(get_post('to') ?? ""); ?>" />
